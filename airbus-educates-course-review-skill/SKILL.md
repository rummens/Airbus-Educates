...
description: >
  ...
  plain examiner titles with friendly script messages), examiner coverage (a check for every command) plus
...
- **Rule (no status emoji in a title):** an `examiner:execute-test` `title:` carries **no** `✅`/`🔍`/`❌` and no `**`. The action block is the state indicator — Educates colours it amber while queued or running, green on pass, red on fail — and `title:` is static plain text with no state-aware property (`name`, `title`, `prefix`, `args`, `timeout`, `retries`, `delay`, `cooldown`, `url`, `inputs`). A tick typed into the title therefore shows green **before**, **during** and **after a failed** run, contradicting the only real signal. **Check:** grep content for `title:` lines inside `examiner:execute-test` blocks containing an emoji or `**`. **Fix:** strip them; name the outcome imperatively and put emphasis in the page prose. *(content-formatting-reference — **Major**: a permanent green tick on a failing check is a false pass signal, the same class of defect as I4.)*
- **Rule (friendly script messages):** every test script prints `✅ …` on success and `❌ …` on failure — that output is produced at the moment of pass/fail, so a marker there is truthful. **Check:** grep `workshop/examiner/tests/*` for `echo` lines with no `✅`/`❌`. **Fix:** add them, keeping the expected/found/next-move detail. *(**Minor**.)*
...
| 30 | Static `✅` in an examiner title showed a green tick before, during and after a failed check | I5, H9 |
