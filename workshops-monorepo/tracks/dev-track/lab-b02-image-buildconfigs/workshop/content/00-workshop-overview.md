...
- **From Docker to Kubernetes on DCS** You know Deployments, and how to apply a
  manifest with `envsubst < file.yaml | oc apply -f -` when it references
  `${DCS_REGISTRY}`.
- Comfort reading a [YAML](https://kubernetes.io/docs/concepts/configuration/overview/) manifest — this lab reuses, rather than re-teaches, the Deployment shape from the **Deploy Your First App** lab/the **From Docker to Kubernetes on DCS** lab.

This lab does **not** re-teach how to write or apply a Deployment, or how to reach a
Service — the **From Docker to Kubernetes on DCS** lab already covered that. It's about the *build* step that produces the image
