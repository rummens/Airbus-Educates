...
- **Deploy / scale / restart** workloads (A02).
- **Configure** with ConfigMaps and Secrets (A03).
- **Expose** an app with a Service and, in a PROD namespace, a Route (A04).
- **Claim storage** with a PVC (A05).
...
- **Image mirroring** — pulling an external image into Harbor (A05 hinted at the air-gap).
- **New repos / catalogs** in Harbor.
- **S3 bucket** — object storage (A05: not a PVC).
...
asides from A04 and A05. Now they have a name and a place.
{{< /note >}}
