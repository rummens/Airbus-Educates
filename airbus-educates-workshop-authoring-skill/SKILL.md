...
- [ ] Every `examiner:execute-test` title starts with `✅` (change) or `🔍` (observation), and no examiner `title:`/`description:` contains `**` (plain text — asterisks show literally)
- [ ] Every examiner script prints `✅ …` on success and `❌ …` naming expected, found, and the next move on failure
...
- [Content Formatting Reference](references/content-formatting-reference.md) - How the content is laid out so it can be skimmed: bold key terms, short one-idea paragraphs, lists for actions and choices, call-out blocks with bold emoji labels, friendly examiner titles and messages, the sanctioned emoji set, and the ConsoleLab plain-text exception
