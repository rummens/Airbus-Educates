...
8. **Teach concepts, don't script commands.** One concept per page; explain what/why/how and trade-offs, show and explain expected output, explain non-obvious flags, and cover the topic completely. Depth beats brevity — split long topics into more workshops rather than thinning explanations. **Write in plain language (no idioms, metaphors, or figurative flourishes); keep commands short and explained (no dense chained one-liners); define term pairs like declarative/imperative the first time they appear; lean on diagrams; and give each lab a slide deck (one slide per page).** See [references/content-depth-reference.md](references/content-depth-reference.md).
9. **Skimmable formatting.** Every learner-facing page is laid out to be *scanned*: **key nouns/verbs bold** on first mention, paragraphs of ≤3 lines with one idea each, long sentences split, actions and choices as **lists**, hints/warnings in **call-out blocks with a bold emoji label** (`{{< note >}}` → `**💡 Tip:**`, `{{< warning >}}` → `**⚠️ Watch out:**`), and examiner **test-script** messages prefixed `✅`/`❌`. **Never put a status emoji or bold in an examiner `title:`** — it is static plain text, and the action block itself is what turns amber/green/red. Applies to instruction pages, `README.md`, `academy.dcs/details`, and slides — **not** to ConsoleLab step text (plain text only). See [references/content-formatting-reference.md](references/content-formatting-reference.md).
10. **End-of-workshop feedback page.** Every workshop ends with a `98-your-feedback.md` page that opens the feedback form (rating + clarity + comment), collected by the DCS Academy portal (`/form`). See [references/feedback-page-reference.md](references/feedback-page-reference.md).
11. **Workshop README / description.** Every workshop's root `README.md` is its pre-launch prospectus — hook, lead, meta (audience/duration/prereqs/format), learning objectives, and what you'll do. It is the rich description shown in the portal course view. Don't hardcode the rating (portal shows it live) and don't double the title (portal strips the README's H1). See [references/workshop-readme-reference.md](references/workshop-readme-reference.md).
...
- **Format for skimming (house standard).** Bold the key nouns/verbs on first mention; keep paragraphs to ≤3 lines and one idea; split long sentences into separate sentences (a line break alone does nothing in Markdown — use a blank line or a list); turn every sequence of actions into a numbered list and every set of options/facts into a bulleted list; put hints and warnings in `{{< note >}}` / `{{< warning >}}` call-outs led by a bold emoji label (`**💡 Tip:**`, `**⚠️ Watch out:**`); keep examiner titles plain (no emoji, no bold — the action block is the state indicator and a static `✅` lies) while examiner **script output** leads with `✅`/`❌`. See [references/content-formatting-reference.md](references/content-formatting-reference.md).
- **Write plainly; keep commands simple; add slides (house standard).** Prefer literal wording over idioms/metaphors; define declarative/imperative (and similar pairs) before using them; split dense chained commands into short explained steps; add a diagram wherever a relationship is taught; and give each lab a `workshop/slides/` deck (one slide per page, with the page's diagram repeated on its slide) reached via a per-page `dashboard:reload-dashboard` action. See [references/content-depth-reference.md](references/content-depth-reference.md) and [references/slides-reference.md](references/slides-reference.md).
...
- [ ] **Pages are formatted to be skimmed** — key terms bold on first mention, paragraphs ≤3 lines/one idea, actions and choices as lists, hints/warnings in call-outs with a bold emoji label, no more than two or three call-outs per page (see [references/content-formatting-reference.md](references/content-formatting-reference.md))
- [ ] No examiner `title:`/`description:` contains a status emoji (`✅`/`🔍`) or `**` — both are static plain text; the action block is the state indicator, so a typed-in tick reads green before, during and after a failed run
- [ ] Every examiner script prints `✅ …` on success and `❌ …` naming expected, found, and the next move on failure; the amber/green/red indicator is explained in prose on the first page with a check
- [ ] `README.md` and `academy.dcs/details` use `> **💡 Tip:** …` blockquote call-outs (Hugo shortcodes do not render there); ConsoleLab step text stays plain text
...
- [Content Formatting Reference](references/content-formatting-reference.md) - How the content is laid out so it can be skimmed: bold key terms, short one-idea paragraphs, lists for actions and choices, call-out blocks with bold emoji labels, plain examiner titles with friendly script messages, the sanctioned emoji set, and the ConsoleLab plain-text exception
- [Content Depth Reference](references/content-depth-reference.md) - The standard that workshops teach concepts (one per page: what/why/how, expected output, flags, complete coverage) rather than scripting commands; plain language (no idioms); simple explained commands; declarative/imperative defined before use
- [Slides Reference](references/slides-reference.md) - The one-slide-per-page deck: the copy-verbatim self-contained renderer, `slides.md` format, putting each page's diagram on its slide, and the `dashboard:reload-dashboard` jump link. Serves in-session and via the portal outside a session
