...
[![coverage report](https://gitlab.com/bvlgtyi4-group/airbus-educates/badges/main/coverage.svg?job=portal-tests&min_good=90)](https://gitlab.com/bvlgtyi4-group/airbus-educates/-/jobs)
...
| [`dcs-academy-portal`](dcs-academy-portal) | Workshops + TrainingPortal + custom UI + OpenShift-OAuth gate + feedback (CNPG) | `dcs-academy-portal`, portal at `academy.<domain>`, backend `dcst-ui`, sessions `dcst-*` |
...
- **Dynamic (user sessions)** → `dcst-*` (from the portal name `dcst`, e.g. `dcst-w01-xxxx`, `...-vc`); the Educates backend runs in `dcst-ui`.
