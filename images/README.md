...
| `dcs-ci:dev` | Python slim + `git` + `curl` + `oc` | CI base for **both tiers** (`.gitlab-ci.yml` stages `test` + `e2e`). Bakes what `portal-tests` + `workshop-static` need so jobs skip per-run `apt-get`. The cluster tier (stage `e2e`) uses the same image for `oc` + `git` + `curl`, so there is no per-runner binary install. `oc` logs into the dev OpenShift cluster from `CI_OC_HOST` / `CI_OC_TOKEN` CI/CD variables. |
